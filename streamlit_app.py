...
# Static UI assets (CSS/HTML blobs) live in ui/assets/ so this file stays
# navigable. Cached forever - the files are frozen at process start.
_ASSETS_DIR = Path(__file__).parent / "ui" / "assets"


@lru_cache(maxsize=None)
def _asset(name: str) -> str:
    """Return the contents of a static asset in ui/assets/ (cached)."""
...
def _parse_iso_date(value: str | None) -> datetime | None:
    """Parse a strict YYYY-MM-DD string; None for empty or non-ISO input
    (relative phrases like 'in 7-10 days' are valid data, not errors)."""
    if not value:
        return None
    try:
...
def _format_date(iso_date: str | None) -> str:
    ...
    than silently dropped; "Date not specified" when None/empty.
    """
    if not iso_date:
        return "Date not specified"
    dt = _parse_iso_date(iso_date)
    return dt.strftime("%B %-d, %Y") if dt else iso_date.strip()


def _date_is_iso(value: str | None) -> bool:
    """True iff `value` looks like a YYYY-MM-DD ISO date the renderer can
    format with a calendar icon. Relative phrases like 'in 7-10 days' return
    False so the renderer picks a clock icon ('timing, not a fixed date')."""
...
def _render_chat_widget(result: dict) -> None:
    ...
    panel_css = _asset("chat_panel.css")
...
def _landing_intro_html() -> str:
    ...
    """
    return _asset("landing_intro.html")
